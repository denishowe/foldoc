...
# Runs in the dictionary source directory

our $dicdir = '.';
BEGIN
{
	push @INC, '.'; # Shared library module, Foldoc.pm, lives in the source dir
}
use Foldoc;
...
my $build_dir = '../Build';
my $log_dir = "$build_dir/logs"; # Pre-processed logs
...
# Read the dictionary source and make FOLDOC indexes and contents pages
# in a subdirectory called contents.
...
<div class="word-list">$subjects</div>
	...
	template("contents/subject.html");
...
sub read_logs
{
	...
	print "\nCheck last $n/" . @wwwlogs . " web logs, $old old logs\n";
...
sub save_log
{
	...
	print STDERR "\n  Saving log $log";
	-d $log_dir or mkdir $log_dir, 0777 or die "Can't mkdir $log_dir: $!";
	open my $LOG, ">", "$log_dir/$log" or die "Can't write $log_dir/$log: $!";
